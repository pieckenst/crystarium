import { Client, Constants, Collection } from 'eris';
import fs from 'fs';
    ...
    initializeManager() {
        try {
            ...
            this.manager
                ...
                let channel = this.getChannel(player.textChannel);
                ...
                const channel = this.getChannel(player.textChannel);
                if (channel && 'createMessage' in channel) {
                    channel.createMessage({
                        embeds: [{
                                color: Math.floor(Math.random() * 0xFFFFFF),
...
client.on("ready", () => {
    try {
        ...
        const slashCommands = Array.from(client.slashCommands.values()).map(command => ({
            ...
            type: 1 // Specify the type as a const assertion
        }));
...
client.on("messageCreate", async (message) => {
    ...
    try {
        if (!message.content || !message.content.startsWith(prefix) || message.author.bot)
            return;
