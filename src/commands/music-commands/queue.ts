...
export default {
  ...
  execute: async (
    ...
    if (!player)
      return msg.channel.createMessage("There is no player for this guild.");
    ...
    const embed: {
      ...
      author: {
        name: `Queue for ${msg.channel instanceof GuildChannel ? msg.channel.guild.name : "Unknown Guild"}`,
      },
      fields: [],
      footer: { text: "" },
    };
    ...
    const page =
      args.length && Number(args[0]) ? Math.max(1, Number(args[0])) : 1;
