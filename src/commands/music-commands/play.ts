    ...
    switch (res.loadType) {
      ...
      case "SEARCH_RESULT":
        ...
        const results = res.tracks
          .slice(0, max)
          .map(
            (track, index) => `${index + 1} - [${track.title}](${track.uri})`,
          )
          .join("\n");
        ...
        await harmonix.client.createMessage(msg.channel.id, {
          embed: {
            ...
            fields: [
              {
                name: "Cancel Search:",
                value: "Type end or any other number to cancel the search",
                inline: true,
              },
            ],
            timestamp: new Date(),
        ...
        const response = await new Promise<Message<TextableChannel>[]>(
          (resolve) => {
            const collector = new Eris.MessageCollector(msg.channel, filter, {
              max: 1,
              time: 30000,
            });
