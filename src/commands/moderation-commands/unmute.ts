...
export default {
    ...
    execute: async (harmonix: Harmonix, msg: Message<TextableChannel>, args: string[]) => {
        ...
        try {
            await harmonix.client.editGuildMember(msg.channel.guild.id, mutemember.id, {
                communicationDisabledUntil: null
            }, muteReason);
            
            await logInfo(`${mutemember.username} was unmuted by ${msg.author.username}`, 'unmute');
            ...
            await logError(`Error unmuting member: ${error}`, error instanceof Error ? error : undefined, 'unmute');
            if (msg.channel.id) {
                await harmonix.client.createMessage(msg.channel.id, {
                    embed: {
                        color: 0xFF0000,
