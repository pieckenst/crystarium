...
class HelpCommand extends Eris.Command {
  constructor() {
    super({
      name: "help",
      description: "Shows the help menu or information about a specific command",
      options: [
        {
          name: "command",
  ...
  generateOptions() {
    ...
    const options = Object.keys(this.categories)
      ...
      .map(category => {
        ...
        return { label: category, value: category, description: "No description" };
      });
    
    if (options.length === 0) {
      options.push({ label: "General", value: "General", description: "No description" });
      console.log("Added default General option");
    }
    
    options.push({ label: "Close", value: "Close", description: "No description" });
