...
export default {
    ...
    execute: async (harmonix: Harmonix, msg: Message<TextableChannel>, args: string[]) => {
        ...
        if (args.length === 0) {
            ...
            if (msg.channel.id) {
                await harmonix.client.createMessage(msg.channel.id, `The player volume is \`${player.volume}\`.`);
            }
            return;
        }
        ...
        if (isNaN(volume) || volume < 1 || volume > 100) {
            ...
            if (msg.channel.id) {
                await harmonix.client.createMessage(msg.channel.id, 'You need to give me a volume between 1 and 100.');
            }
            return;
        }
