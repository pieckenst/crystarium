  ...
  async execute(
    ...
    if (args.length === 0) {
      await harmonix.client.createMessage(
        msg.channel.id,
        "Please provide a server and character name, or a Lodestone ID.",
      );
      return;
    }
    ...
    try {
      ...
      const waitMessage = await harmonix.client.createMessage(msg.channel.id, {
      ...
      if (info.bozja && info.bozja.level) {
        mainEmbed.fields.push({
          name: "Bozja",
          value: `${info.bozja.name} (Level ${info.bozja.level}, Mettle: ${info.bozja.mettle})`,
          inline: true,
        });
      }
      ...
      if (info.eureka && info.eureka.level) {
        mainEmbed.fields.push({
          name: "Eureka",
          value: `${info.eureka.name} (Level ${info.eureka.level}, EXP: ${info.eureka.exp})`,
          inline: true,
        });
      }
