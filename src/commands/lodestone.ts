...
export default class extends defineCommand({
    ...
    static async execute(harmonix: Harmonix, interaction: CommandInteraction | Message<TextableChannel>, args: { server?: string, character: string }) {
        ...
        try {
            ...
            let responseMessage;
            if (interaction instanceof CommandInteraction) {
                responseMessage = await interaction.createFollowup({ embeds: [pleaseWaitEmbed] });
            } else {
                responseMessage = await harmonix.client.createMessage(interaction.channel.id, { embed: pleaseWaitEmbed });
            }
            if (harmonix.options.debug) {
            ...
            if (harmonix.options.debug) {
                console.debug('[Lodestone] Debug: Fetching character info for ID:', characterId);
            }
            const info = await fetchCharacterInfo(characterId);
            const fetchTime = (Date.now() - startTime) / 1000;
            if (harmonix.options.debug) {
                console.debug('[Lodestone] Debug: Character info fetched in', fetchTime, 'seconds');
            }

            if (info.jobIconUrl && info.jobIconUrl.trim() !== '') {
                if (harmonix.options.debug) {
                    console.debug('[Lodestone] Debug: Fetching job icon from URL:', info.jobIconUrl);
                }
                ...
                if ('guild' in interaction.channel) {
                    if (harmonix.options.debug) {
                        console.debug('[Lodestone] Debug: Attempting to create emoji for job:', info.jobName);
                    }
                    try {
                        const jobNameForEmoji = info.classLevels[info.activeClassJob]?.jobname || info.jobName; 
                        if (harmonix.options.debug) {
                            console.debug('[Lodestone] Debug: Job name for emoji:', jobNameForEmoji);
                        }
                        ...
                        if (harmonix.options.debug) {
                            console.debug('[Lodestone] Debug: Emoji created successfully. Emoji ID:', emoji.id);
                        }
                    } catch (error) {
                        if (error.message.includes('Invalid Form Body') && error.message.includes('image: Invalid image data')) {
                            if (harmonix.options.debug) {
                                console.debug('[Lodestone] Debug: Skipping emoji creation due to invalid image data');
                            }
                        } else {
                            console.error('[Lodestone] Error: Failed to create emoji:', error.message);
                        }
                    }
                } else {
                    if (harmonix.options.debug) {
                        console.debug('[Lodestone] Debug: Channel does not have a guild, skipping emoji creation');
                    }
                }
            } else {
                if (harmonix.options.debug) {
                    console.debug('[Lodestone] Debug: No active class job found, skipping emoji creation');
                }
            }
            ...
            if (interaction instanceof CommandInteraction) {
                await interaction.editMessage(responseMessage.id, { embeds: [mainEmbed, classLevelEmbed] });
            } else {
                await harmonix.client.editMessage(interaction.channel.id, responseMessage.id, { embeds: [mainEmbed, classLevelEmbed] });
            }
            if (harmonix.options.debug) {
            ...
            await this.sendErrorMessage(harmonix, interaction, 'An error occurred while fetching character information. Please try again later.');
        }
    }

    static createMainEmbed(info: CharacterInfo, characterId: string, emoji: any, fetchTime: number) {
        return {
    ...
    static createClassLevelEmbed(info: CharacterInfo) {
        ...
        for (const category of categories) {
            ...
            const categoryJobs = Object.entries(info.classLevels)
                ...
                .sort(([, a], [, b]) => Number((b as ClassJobLevel).level) - Number((a as ClassJobLevel).level));
            ...
            if (categoryJobs.length > 0) {
                ...
                classLevelEmbed.fields.push({
                    ...
                    value: categoryJobs.map(([, data]) => {
                        console.debug(`Processing job: ${(data as ClassJobLevel).jobname}`);
                        const levelDisplay = (data as ClassJobLevel).level === 'Class not unlocked' ? 'Class not unlocked' : `Level ${(data as ClassJobLevel).level}`;
    ...
    static async sendErrorMessage(harmonix: Harmonix, interaction: CommandInteraction | Message<TextableChannel>, message: string) {
        if (harmonix.options.debug) {
