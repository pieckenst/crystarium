      ...
      Effect.tryPromise(async () => {
        ...
        switch (flag) {
          case "disableCommand":
            ...
            break;
          case "enableCommand":
            harmonix.options.featureFlags.disabledCommands =
              harmonix.options.featureFlags.disabledCommands.filter(
                (cmd) => cmd !== value,
              );
            break;
          case "setBetaCommand":
          ...
          case "unsetBetaCommand":
            harmonix.options.featureFlags.betaCommands =
              harmonix.options.featureFlags.betaCommands.filter(
                (cmd) => cmd !== value,
              );
            break;
          case "setDatabase":
            harmonix.options.featureFlags.useDatabase = value as
              | "sqlite"
              | "postgres"
              | "none";
            break;
        ...
        if (harmonix.options.database) {
          await harmonix.options
            .database("featureFlags")
            .insert(harmonix.options.featureFlags);
        }
