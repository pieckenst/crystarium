...
export default class extends defineCommand({
  ...
  static async execute(
    ...
    args: { type: string; status: string } | string[],
  ) {
    ...
    if (harmonix.options.debug) {
      console.log(
        `Debug: Interaction type: ${interaction instanceof CommandInteraction ? "CommandInteraction" : "Message"}`,
      );
      console.log(`Debug: Args:`, args);
    }

    if (interaction instanceof CommandInteraction) {
      const typeOption = interaction.data.options?.find(
        (opt) => opt.name === "type",
      );
      ...
      if (
        ...
        "value" in typeOption &&
        statusOption &&
        "value" in statusOption
      ) {
        statusType =
          Constants.ActivityTypes[
            typeOption.value as keyof typeof Constants.ActivityTypes
          ];
      ...
      statusType =
        Constants.ActivityTypes[
          type.toUpperCase() as keyof typeof Constants.ActivityTypes
        ];
  ...
  static getStatusType(type: string): BotActivityType {
    switch (type.toUpperCase()) {
      case "PLAYING":
        return Constants.ActivityTypes.GAME;
      case "STREAMING":
        return Constants.ActivityTypes.STREAMING;
      case "LISTENING":
        return Constants.ActivityTypes.LISTENING;
      case "WATCHING":
        return Constants.ActivityTypes.WATCHING;
      case "COMPETING":
        return Constants.ActivityTypes.COMPETING;
      default:
        return Constants.ActivityTypes.GAME;
    }
  }
