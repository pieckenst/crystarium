...
import {
  defineClientStart,
  HarmonixBuilderConfig,
} from "./code-utils/defininingclientstart";
...
async function initHarmonix(): Promise<Harmonix> {
  ...
  const harmonix = await Effect.runPromise(
    clientStart.buildHarmonix(harmonixBuilderConfig),
  );
...
async function loadCommands(harmonix: Harmonix): Promise<void> {
      ...
      Effect.tryPromise({
        try: async () => {
          ...
          Effect.sync(() => {
            ...
            consola.warn(
              colors.yellow(
                `An error occured in : ${file} - Bot will continue running`,
              ),
            );
          }),
      }),
  ...
  consola.info(
    colors.green(
      ` Loaded ${harmonix.commands.size} regular commands and ${harmonix.slashCommands.size} slash commands.`,
    ),
  );
}
      ...
      Effect.tryPromise({
        try: async () => {
          ...
          harmonix.client.on(
            ...
            (...args: any[]) => event.execute(harmonix, ...args),
          );
  ...
  await Effect.runPromise(
    Effect.tryPromise({
      try: async () => {
        ...
        await Effect.runPromise(
          Effect.tryPromise(() => harmonix.client.connect()),
        );
      },
      catch: (error: Error) => {
