  ...
  return Effect.runPromise(
    Effect.tryPromise(async () => {
      ...
      switch (level) {
        case LogLevel.INFO:
          consola.info(
            colors.blue(`${contextInfo}${commandInfo}${formattedMessage}`),
          );
          break;
        case LogLevel.SUCCESS:
          consola.success(
            colors.green(`${contextInfo}${commandInfo}${formattedMessage}`),
          );
          break;
        case LogLevel.WARN:
          consola.warn(
            colors.yellow(`${contextInfo}${commandInfo}${formattedMessage}`),
          );
          break;
        case LogLevel.ERROR:
          consola.error(
            colors.red(`${contextInfo}${commandInfo}${formattedMessage}`),
          );
          if (error) {
        ...
        case LogLevel.DEBUG:
          consola.debug(
            colors.magenta(`${contextInfo}${commandInfo}${formattedMessage}`),
          );
          if (data) {
            consola.debug(
              colors.magenta("Debug data:"),
